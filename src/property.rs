use leptos::tachys::html::style::{IntoStyleValue, style};
...
impl From<Style> for StyleProperty {
    fn from(style: Style) -> Self {
        std::borrow::Cow::Borrowed(style.as_str())
    }
}

/// Create a typed style property struct that maps to a [`Style`] variant.
///
/// Typed style properties can be used in hook attribute tuples for Leptos attribute spreading.
/// Each generated struct implements `AsRef<str>` (returning the CSS property name) and provides
/// a `with_value` method for creating a `leptos::tachys::html::style::Style` tuple.
///
/// # Example
/// ```ignore
/// typed_style!(TouchActionStyle, TouchAction, "touch-action");
///
/// // Use in hook attribute tuples:
/// pub type UsePressAttrs = (
///     leptos::tachys::html::style::Style<(TouchActionStyle, &'static str)>,
/// );
///
/// TouchActionStyle::with_value("pan-x pan-y pinch-zoom")
/// ```
macro_rules! typed_style {
    ($name:ident, $variant:ident, $css_name:literal) => {
        #[doc = concat!("Typed marker for the `", $css_name, "` CSS style property.")]
        ///
        /// Use with [`with_value`](Self::with_value) to create a spreadable style attribute for Leptos views.
        #[derive(Debug, Clone, Copy)]
        pub struct $name;

        impl AsRef<str> for $name {
            fn as_ref(&self) -> &str {
                Style::$variant.as_ref()
            }
        }

        impl $name {
            /// Create a Leptos style attribute tuple with this property and the given value.
